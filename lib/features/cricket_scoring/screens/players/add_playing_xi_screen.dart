// lib\features\cricket_scoring\screens\players\add_playing_xi_screen.dart
...
class AddPlayingXIScreen extends StatefulWidget {
  final MatchModel match;
  final TeamModel teamA;
  final TeamModel teamB;

  const AddPlayingXIScreen({
    super.key,
    required this.match,
    required this.teamA,
    required this.teamB,
  ...
  @override
  State<AddPlayingXIScreen> createState() => _AddPlayingXIScreenState();
}

class _AddPlayingXIScreenState extends State<AddPlayingXIScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadPlayers() async {
    try {
      print('Loading players for Team A: ${widget.teamA.id} (${widget.teamA.name})');
      print('Loading players for Team B: ${widget.teamB.id} (${widget.teamB.name})');
      ...
      print('Team A players loaded: ${teamAPlayers.length}');
      print('Team B players loaded: ${teamBPlayers.length}');
      ...
      if (teamAPlayers.isEmpty || teamBPlayers.isEmpty) {
        ...
        final filteredTeamAPlayers = allPlayers.where((p) => p.teamid == widget.teamA.id).toList();
        final filteredTeamBPlayers = allPlayers.where((p) => p.teamid == widget.teamB.id).toList();

        print('Filtered Team A players: ${filteredTeamAPlayers.length}');
        print('Filtered Team B players: ${filteredTeamBPlayers.length}');

        if (mounted) {
          setState(() {
            _teamAPlayers = filteredTeamAPlayers;
            _teamBPlayers = filteredTeamBPlayers;
            _isLoading = false;
          });
        }
      } else {
  ...
  Future<void> _savePlayingXI() async {
    ...
    try {
      ...
      final existingTeamA = await _databaseService.getPlayingXIByMatchAndTeam(widget.match.id, widget.teamA.id);
      final existingTeamB = await _databaseService.getPlayingXIByMatchAndTeam(widget.match.id, widget.teamB.id);

      for (final playingXI in [...existingTeamA, ...existingTeamB]) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Playing XI saved successfully!'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error saving playing XI: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget _buildEmptyState({
    ...
    return Center(
          child: Padding(
            padding: EdgeInsets.all(MediaQuery.of(context).size.width < 360 ? 24 : 40),
        child: Column(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Team A: ${widget.teamA.name}\nTeam B: ${widget.teamB.name}',
            textAlign: TextAlign.center,
            style: TextStyle(
              fontSize: 14,
              color: Colors.grey[500],
    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton.icon(
          onPressed: _canSave() && !_isSaving ? _savePlayingXI : null,
          icon: _isSaving
              ? SizedBox(
            width: 20,
            height: 20,
          ...
          label: Text(
            _isSaving ? 'Saving...' : 'Save Playing XI',
            style: const TextStyle(
              fontSize: 16,
              fontWeight: FontWeight.bold,
            ),
